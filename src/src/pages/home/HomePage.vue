...
.date-text { font-size: 16px; opacity: 0.8; margin: 12px 0 24px; font-family: monospace; }
...
/* 3. 指标卡片样式 */
.dashboard-grid { display: flex; flex-direction: column; gap: 40px; }
...
.label { font-size: 14px; color: var(--text-regular); }
.value { font-size: 28px; font-weight: 700; color: var(--text-primary); margin: 4px 0; }

/* 4. 底部面板与玻璃拟态 */
.bottom-layout { display: grid; grid-template-columns: 1fr 1.5fr; gap: 32px; }
...
.panel-header { display: flex; align-items: center; gap: 8px; font-weight: 700; color: var(--text-primary); }
...
.entry-item:hover { background: rgba(64, 158, 255, 0.1); }
.entry-label { color: var(--text-regular); font-size: 14px; }
...
/* 渐变图标适配暗黑模式 */
:global(html.dark) .icon-circle { filter: brightness(0.85); }
...
/* 动画部分 */
.pulse-dot {
  width: 8px; height: 8px; background: #52c41a; border-radius: 50%;
