...
WITH InRegBase AS(
    ...
    WHERE a.AuditorDate >= @startTime AND a.AuditorDate <= @endTime AND a.Status = 21 AND a.WarehouseType IN (0,1) AND a.Source = 1
),
InboundBase AS (
    ...
    WHERE a.billstate IN ('41','51') AND b.IsVoid = 0 AND a.EnterDate >= @startTime AND a.EnterDate <= @endTime
    UNION ALL
    ...
    WHERE a.Status = 21 AND a.WarehouseType IN (0,1) AND a.Source = 1 AND a.ConfirmerDate >= @startTime AND a.ConfirmerDate <= @endTime
),
OutboundBase AS (
    ...
    WHERE a.Status IN ('21','51','61') AND b.IsVoid = 0 AND a.TreasuryDepartment IN ('200346','200418','200632','200624') AND a.BLDate >= @startTime AND a.BLDate <= @endTime
    UNION ALL
    ...
    WHERE dr.IsVoid = 0 AND d.Source = '1' AND d.IsStockGoods = '0' AND d.[Type] IN ('1','2') AND d.[Status] IN (61,71,41,81,22,91,19,29,99) AND dr.CreateTime >= @startTime AND dr.CreateTime <= @endTime
),
ReturnBase AS (
    ...
    WHERE a.BLDate >= @startTime AND a.BLDate <= @endTime AND a.StorehouseID IN ('200346','200418','200632','200624') AND a.Status = 21 AND b.IsVoid = 0
    UNION ALL
    ...
    WHERE a.Source in (0,3) AND a.Status in (21,61,51) AND AuditorDate >= @startTime AND AuditorDate <= @endTime
),
PurchaseBase AS (
    ...
    WHERE a.Status in (91,71,61,81,92,51) AND a.AuditorDate >= @startTime AND a.AuditorDate < @endTime AND HRcODE <> '0000900000'
),
PushBase AS (
    ...
    WHERE a.PushDate >= @startTime AND a.PushDate < @endTime
),
RefundBase AS (
    ...
    WHERE A.Status IN (51) AND A.SourceAuditorDate >= @startTime AND A.SourceAuditorDate <= @endTime
),
FinalResults AS (
